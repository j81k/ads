...
function getPosts($this) {
    ...
    switch(type) {
        ...
        case 'sub-cat' :
            data.data.termId = $this.attr('data-term-id');
            data.data.postType = $this.attr('data-post-type');
        break;    
    ...
    $.post( siteUrl + 'ajax/common.php', data, function( results ){
        ...
        var $plotter = $this.attr('data-plotter') || $this;
        $plotter.html( results );
        
        var hidden = JSON.parse($('.ajax-hidden:last').val())
        ,   perPage = hidden['perPage'] || 10;
        
        $this.attr('data-offset', data.data.offset + perPage);
...
$(document).on('ready', function(){
    ...
    if( $('#sub-cat-list').length > 0 ) {
        getPosts($('#sub-cat-list'));
    }
